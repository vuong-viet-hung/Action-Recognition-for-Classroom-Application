...
class ActionKeypointsGenerator(keras.utils.Sequence):
    def __init__(
        self, data_paths: List[str], string_labels: List[str], classes: List[str], batch_size: int
    ) -> None:
    ...
    def __getitem__(self, idx) -> Tuple[np.ndarray, np.ndarray]:
        n_batches = self.data_size // self.batch_size
        batch_start = self.batch_size * idx
        batch_end = self.batch_size * (idx + 1) if idx < n_batches else self.data_size

        keypoints = []
        labels = []

        for data_path, label in zip(
            self.data_paths[batch_start:batch_end], self.encoded_labels[batch_start:batch_end]
        ):
...
def sample_classes(data_path: str, n_classes: int) -> Path:
    ...
    for selected_class_directory in random.sample(classes_directories, n_classes):
        yield selected_class_directory


def sample_data(selected_classes_directories: List[Path]) -> Path:
    ...
    for selected_class_directory in selected_classes_directories:
        for selected_video in selected_class_directory.iterdir():
            matches = sample_regex.search(selected_video.name)
            # Video's information contains its action and group
            video_infos = (matches.group(1), matches.group(2))
            # Append samples from different groups for more data diversity
            if matches in existing_groups:
                continue
            existing_groups.add((matches.group(1), matches.group(2)))
            yield selected_video
